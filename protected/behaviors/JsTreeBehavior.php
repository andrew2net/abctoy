...
class JsTreeBehavior extends CBehavior {
  ...
  public function actionCreateRoot() {
    if (isset($_POST[$this->modelClassName])) {
      ...
      $new_root->name = $_POST[$this->modelClassName]['name'];
      if ($new_root->saveNode(false)) {
        $this->saveNode($_POST[$this->modelClassName], $new_root);
        echo json_encode(array('success' => true,
  ...
  public function actionCreateNode() {
    if (isset($_POST[$this->modelClassName])) {
      ...
      $model->name = $_POST[$this->modelClassName]['name'];
      $parent = $this->loadModel($_POST['parent_id']);
      if ($model->appendTo($parent)) {
        $this->saveNode($_POST[$this->modelClassName], $model);
        echo json_encode(array('success' => true,
          'id' => $model->primaryKey)
        );
        exit;
  ...
  private function saveNode($post_node, Category &$model) {
    ...
    if ($post_node['url'] != $model->url) {
      if (strlen($post_node['url']) > 0) {
        $uploaded_file = Yii::getPathOfAlias('webroot') . $post_node['url'];
        if (file_exists($uploaded_file)) {
          if ($model->isNewRecord)
            $model->saveNode(FALSE);
          $ext = substr($post_node['url'], strrpos($post_node['url'], '.'));
          $file_name = Yii::getPathOfAlias('webroot') . '/images/category/' . $model->id . $ext;
          rename(Yii::getPathOfAlias('webroot') . $post_node['url'], $file_name);
        }
      }
//      if (strlen($model->url) > 0 && file_exists($old_file))
//        unlink($old_file);
      if (isset($file_name))
