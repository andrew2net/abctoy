...
class NestedSetBehavior extends CActiveRecordBehavior {
  ...
  public function moveAsRoot() {
    ...
    try {
      ...
      $owner->updateAll(
        ...
        $db->quoteColumnName($this->rootAttribute) . '=' . CDbCriteria::PARAM_PREFIX . CDbCriteria::$paramCount, array(CDbCriteria::PARAM_PREFIX . CDbCriteria::$paramCount++ => $owner->{$this->rootAttribute}));
  ...
  public function moveRoot($previous_node_id, $next_node_id) {
    /* @var $owner Category */
    /* @var $owner NestedSetBehavior */
    $owner = $this->getOwner();
    $id = $owner->id;
    $next_id = Category::model()->find(array('select' => 'max(id) as max_id'))->max_id + 1;
    $tr = Yii::app()->db->beginTransaction();
    try {
      if ($next_node_id != 'false') {
        $next_nodes = Category::model()->findAll(array(
          'condition' => 'level=1 AND id>=:id AND id<>:owner_id',
          'order' => 'id DESC',
          'params' => array(':id' => $next_node_id, ':owner_id' => $id),
        ));
        foreach ($next_nodes as $node) {
          $next_id = $this->updateRoot($node, $next_id);
        }
        $this->updateRoot($owner, $next_node_id);
      }  else {
        $this->updateRoot($owner, $next_id);
      }
    } catch (Exception $ex) {
      $tr->rollback();
  ...
  private function updateRoot($node, $nex_id) {
    /* @var $node Category */
    /* @var $node NestedSetBehavior */
    $curr_id = $node->id;
    $node->id = $nex_id;
    $node->saveNode(FALSE);
    $scats = Category::model()->findAllByAttributes(array('root' => $curr_id));
    foreach ($scats as $cat) {
      /* @var $cat Category */
      /* @var $cat NestedSetBehavior */
      $cat->root = $nex_id;
      $cat->saveNode(FALSE);
    }
    return $curr_id;
  ...
  private function moveNode($target, $key, $levelUp) {
    ...
    try {
      ...
      if ($this->hasManyRoots && $owner->{$this->rootAttribute} !== $target->{$this->rootAttribute}) {
        foreach (array($this->leftAttribute, $this->rightAttribute) as $attribute) {
          $owner->updateAll(array($attribute => new CDbExpression($db->quoteColumnName($attribute) . sprintf('%+d', $right - $left + 1))), $db->quoteColumnName($attribute) . '>=' . $key . ' AND ' . $db->quoteColumnName($this->rootAttribute) . '=' . CDbCriteria::PARAM_PREFIX . CDbCriteria::$paramCount, array(CDbCriteria::PARAM_PREFIX . CDbCriteria::$paramCount++ => $target->{$this->rootAttribute}));
        }
